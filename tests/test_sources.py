...
from sources import get_source, list_sources, register_source, get_all_sources
from sources.base import FailureSource
from sources.logfile import LogFileSource
...
class TestSourceRegistry:
    ...
    def test_get_all_sources_no_config(self):
        ...
        assert len(sources) >= 3  # At least the 3 built-ins
...
class TestLogFileSource:
    ...
    def test_scan_log_with_errors(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(results) == 2  # ERROR and FATAL lines
    ...
    def test_scan_respects_time_window(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            # Create a log file but set mtime to the past
            ...
            source = LogFileSource(paths=[f"{tmpdir}/*.log"])
            results = source.scan(hours=1)  # Only last hour
    ...
    def test_severity_classification(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            results = source.scan(hours=24)
            severities = {r["error"].split()[0]: r["severity"] for r in results}
            # FATAL line should be critical
...
class TestJSONLSource:
    ...
    def test_scan_jsonl_with_errors(self):
        with tempfile.NamedTemporaryFile(mode="w", suffix=".jsonl", delete=False) as f:
            ...
            assert severities["1"] == "warning"  # error -> warning
            assert severities["2"] == "critical"  # fatal -> critical
