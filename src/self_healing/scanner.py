"""
Scanner module — scan for failures across multiple sources using a plugin architecture.
Includes cascading failure detection — groups related failures that likely share a root cause.

Refactored from scripts/scan-failures.py for package use.
"""
...
def load_config(config_path=None):
    ...
    # Auto-detect config in common locations
    for candidate in [
        Path("self-healing.yaml"),
        Path("self-healing.yml"),
        Path("self-healing.json"),
    ]:
        if candidate.exists():
...
def run_scan(hours=6, output_json=False, source_names=None, config_path=None):
    """
    Main scan logic. Returns (all_failures, cascades) tuple.
    If output_json or not, also prints results.
    """
    ...
    return all_failures, cascades


def main():
    """CLI entry point for scan-failures (backward compat)."""
    hours = 6
    output_json = False
    source_names = []
    config_path = None
    ...
    while i < len(args):
        if args[i] == "--hours" and i + 1 < len(args):
            ...
            print("Usage: scan-failures [--hours N] [--json] [--source NAME] [--config PATH]")
            sys.exit(0)
        else:
            i += 1

    run_scan(hours=hours, output_json=output_json, source_names=source_names, config_path=config_path)
