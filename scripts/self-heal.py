...
Commands:
  check "<error message>"     Search for matching known fix (smart multi-signal matching)
  log --error "..." --cause "..." --fix "..." --fix-type heal [--files-changed f1,f2] [--commit hash] [--risk-level low|medium|high|critical]
  list                        List all known fixes
  stats                       Show self-healing statistics
  risk "<description>"        Score the risk of applying a fix to a system

The known-fixes database lives at WORKSPACE/known-fixes.json
Risk profiles live at WORKSPACE/risk-profiles.json (auto-created on first use)
"""
...
# ─── Matching Engine (v2) ──────────────────────────────────────────────────────
# Multi-signal scoring: regex, substring, token overlap, error class, path similarity,
# n-gram overlap. Each signal contributes independently; final score = weighted max.

# Common error class signatures — errors that share a class are likely related
ERROR_CLASSES = {
...
def smart_match(error_text, pattern, pattern_regex=None):
    ...
    Signals (weighted):
    1. Regex match (0.90) — pattern author wrote a specific regex
    2. Exact substring (0.95) — pattern is literally contained in error
    3. Token overlap (up to 0.80) — shared words
    4. Error class match (0.75) — same error category (FileNotFoundError, etc.)
    5. Path similarity (0.70) — similar file paths in both errors
    6. N-gram overlap (up to 0.85) — character-level structural similarity
    """
    signals = []
    ...
    if error_classes and pattern_classes:
        ...
        if class_overlap:
            # Same error class is a strong signal
    ...
    if error_paths and pattern_paths:
        # Check if any path components overlap (e.g., both reference /tmp/ or same script name)
        error_parts = set()
    ...
    # Signal 6: N-gram overlap (character-level structure)
    ...
    # Final score: weighted combination — take the max signal but boost if multiple signals agree
    signals.sort(key=lambda s: s[1], reverse=True)
    best_score = signals[0][1]

    # Agreement bonus: if 3+ signals fire, boost by up to 0.05
    if len(signals) >= 3:
...
def score_risk(description, fix_type="heal"):
    ...
    Factors:
    - System profile match (what kind of system is affected)
    - Fix type (retry=low, patch=medium, heal=higher since it changes code)
    - Reversibility
    - User-facing impact
    - Data loss potential
    """
    ...
    # Sort by keyword hits (most specific match)
    matched.sort(key=lambda x: x[2], reverse=True)

    if not matched:
        # Unknown system — moderate default risk
        base_risk = 0.5
...
def cmd_check(error_text):
    ...
    if matches:
        ...
        # Still provide error classification for unknown errors
        error_classes = classify_error(error_text)
        print(json.dumps({
            "match": False,
...
def cmd_log(args):
    ...
    for entry in db:
        score, _ = smart_match(params["error"], entry.get("pattern", ""), entry.get("patternRegex"))
        if score >= 0.85:
    ...
    pattern_regex = re.sub(r'[/\\][\w._-]+', r'[/\\\\][\\w.-]+', re.escape(error_text))
    error_classes = classify_error(error_text)
...
def main():
    ...
    if cmd == "check":
        ...
        if len(sys.argv) < 3:
            print("Usage: self-heal.py risk \"<description of system/fix>\"")
            sys.exit(1)
        cmd_risk(sys.argv[2])
    else:
